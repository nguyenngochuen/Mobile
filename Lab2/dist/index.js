...
//10
simulateTask(1000).then(message => console.log(message)).catch(err => console.log(err)).finally(() => {
    console.log("Done");
});
const bAsync = new Promise((resolve) => {
    setTimeout(() => {
        resolve("---------Bai B------------");
    }, 2001);
});
...
async function testErr() {
    try {
        await new Promise((_, reject) => setTimeout(() => reject("Loi(B)"), 1000));
    }
    catch (err) {
        console.log("B", err);
    }
